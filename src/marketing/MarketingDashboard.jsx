  ...
  return (
    <div className="bg-white dark:bg-[#11131C] border border-[#E4E7EF] dark:border-[#1E2133] rounded-2xl p-3">
      <div className="flex flex-wrap items-center gap-2">
        <span className="text-[10px] font-bold uppercase tracking-wider text-[#8B92A9] shrink-0">Date range</span>
  ...
  return(
    <div className="bg-white dark:bg-[#11131C] border border-[#E4E7EF] dark:border-[#1E2133] rounded-2xl overflow-hidden">
      <div className="flex items-center gap-3 px-4 py-3.5 cursor-pointer hover:bg-[#F8F9FC] dark:hover:bg-white/[0.02] transition-colors" onClick={()=>setOpen(!open)}>
        <div className="w-9 h-9 rounded-xl flex items-center justify-center text-white font-bold text-[13px] shrink-0" style={{background:COLORS[idx%COLORS.length]}}>{idx+1}</div>
        <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center gap-2 shrink-0 flex-wrap justify-end">
          <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${statusActive?"bg-emerald-50 text-emerald-700 dark:bg-emerald-950/30":"bg-slate-100 text-slate-500 dark:bg-white/5"}`}>{statusActive?"Active":"Paused"}</span>
          {isConfigured&&hasSpend&&(<span className={`inline-flex items-center gap-1 text-[10px] font-bold px-2 py-0.5 rounded-full ${perfBg}`} style={{color:perfColor}}><PerfIcon className="w-3 h-3"/>{perf}</span>)}
      ...
      {isConfigured?(
        <div className="grid grid-cols-4 sm:grid-cols-8 bg-[#F8F9FC] dark:bg-[#0D0F14] border-t border-[#E4E7EF] dark:border-[#1E2133] divide-x divide-[#E4E7EF] dark:divide-[#1E2133]">
          {[["Spend",hasSpend?`₹${Number(m.spend).toLocaleString("en-IN",{maximumFractionDigits:0})}`:"—"],["Impr.",hasSpend?Number(m.impressions||0).toLocaleString("en-IN"):"—"],["Reach",hasSpend?Number(m.reach||0).toLocaleString("en-IN"):"—"],["Clicks",hasSpend?Number(m.clicks||0).toLocaleString("en-IN"):"—"],["CTR",hasSpend?`${Number(m.ctr||0).toFixed(2)}%`:"—"],["CPM",hasSpend?`₹${Number(m.cpm||0).toFixed(2)}`:"—"],["Leads",camp.leads||0],["Converted",camp.converted||0]].map(([k,v])=>(
        ...
        <div className="px-4 py-4 border-t border-[#E4E7EF] dark:border-[#1E2133] space-y-3">
          {isConfigured&&(
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
              {[{label:"Cost per Lead",value:camp.costPerLead?`₹${Number(camp.costPerLead).toLocaleString("en-IN",{maximumFractionDigits:2})}`:"—",tint:"#6366F1"},{label:"Conv. Rate",value:camp.leads>0?`${convRate}%`:"—",tint:"#10B981"},{label:"Cost per Conv.",value:camp.costPerConversion?`₹${Number(camp.costPerConversion).toLocaleString("en-IN",{maximumFractionDigits:2})}`:"—",tint:"#F59E0B"},{label:"Frequency",value:hasSpend?`${Number(m.frequency||0).toFixed(1)}×`:"—",tint:Number(m.frequency)>5?"#EF4444":"#0EA5E9"}].map(c=>(
...
function MetaCampaignsTab({from,to,refreshKey}){
  ...
  // FIX: refreshKey from parent triggers reload on every global refresh
  useEffect(()=>{load(false);},[load,refreshKey]);

  const allCamps=data?.campaigns||[];
  const categories=useMemo(()=>{const cats=new Set(allCamps.map(c=>c.category||"").filter(Boolean));return Array.from(cats).sort();},[allCamps]);
  const camps=useMemo(()=>catFilter?allCamps.filter(c=>(c.category||"")=== catFilter):allCamps,[allCamps,catFilter]);
  const byCategory=useMemo(()=>{const map={};camps.forEach(c=>{const k=c.category||"Uncategorised";if(!map[k])map[k]=[];map[k].push(c);});return Object.entries(map).sort((a,b)=>a[0]==="Uncategorised"?1:b[0]==="Uncategorised"?-1:a[0].localeCompare(b[0]));},[camps]);
  const t=data?.totals||{};
    ...
    <div className="space-y-4">
      <div className="flex items-center gap-3 flex-wrap">
        ...
        <div className="ml-auto flex items-center gap-2">
          {loading&&<span className="inline-flex items-center gap-1 text-[11px] text-[#8B92A9]"><Loader2 className="w-3.5 h-3.5 animate-spin"/>Updating…</span>}
      ...
      {data&&(t.spend>0||t.leads>0)&&(
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5">
          {[{icon:IndianRupee,label:"Total Spend",value:`₹${Number(t.spend||0).toLocaleString("en-IN",{maximumFractionDigits:0})}`,tint:"#EF4444"},{icon:Eye,label:"Impressions",value:Number(t.impressions||0).toLocaleString("en-IN"),tint:"#6366F1"},{icon:Users,label:"Reach",value:Number(t.reach||0).toLocaleString("en-IN"),tint:"#10B981"},{icon:MousePointerClick,label:"Clicks",value:Number(t.clicks||0).toLocaleString("en-IN"),tint:"#0EA5E9"},{icon:Target,label:"Leads",value:t.leads||0,tint:"#F59E0B"},{icon:CheckCircle2,label:"Converted",value:t.converted||0,tint:"#10B981"},{icon:IndianRupee,label:"Cost per Lead",value:t.costPerLead?`₹${t.costPerLead}`:"—",tint:"#8B5CF6"},{icon:Percent,label:"Conv. Rate",value:t.conversionRatePct!=null?`${t.conversionRatePct}%`:"—",tint:"#EC4899"}].map(c=>(
      ...
      {data&&(
        <div className="bg-white dark:bg-[#11131C] border border-[#E4E7EF] dark:border-[#1E2133] rounded-2xl overflow-hidden">
          <div className="flex items-center gap-2.5 px-5 py-3.5 border-b border-[#E4E7EF] dark:border-[#1E2133] bg-gradient-to-r from-indigo-50 to-violet-50 dark:from-indigo-950/20 dark:to-violet-950/20">
            ...
            <div className="flex-1"><p className="text-[13px] font-bold text-[#0F1117] dark:text-[#DDE1F5]">AI Campaign Analysis</p><p className="text-[10px] text-[#8B92A9]">Click "Generate AI Analysis" above for insights and recommendations</p></div>
          </div>
          <div className="p-5">
            ...
            {!aiLoading&&!ai&&<div className="flex flex-col items-center py-8 text-center"><Zap className="w-8 h-8 text-[#C4C9DA] mb-2"/><p className="text-[13px] font-semibold text-[#0F1117] dark:text-[#DDE1F5]">No analysis yet</p><p className="text-[12px] text-[#8B92A9] mt-0.5">Click "Generate AI Analysis" to get recommendations.</p></div>}
            {!aiLoading&&ai&&(
              <div className="space-y-4">
                ...
                {ai.suggestions&&ai.suggestions.length>0&&<div className="rounded-xl bg-amber-50 dark:bg-amber-950/10 border border-amber-200 dark:border-amber-800/30 p-4"><p className="text-[10px] font-bold uppercase tracking-wider text-amber-700 dark:text-amber-400 mb-2 flex items-center gap-1"><Zap className="w-3 h-3"/>Suggestions</p><ul className="space-y-2">{ai.suggestions.map((s,i)=><li key={i} className="flex items-start gap-2 text-[12px] text-[#4B5168] dark:text-[#9DA3BB]"><span className="w-4 h-4 rounded-full bg-amber-200 dark:bg-amber-800/40 text-[9px] font-bold flex items-center justify-center shrink-0 mt-0.5">{i+1}</span>{s}</li>)}</ul></div>}
              </div>
            )}
          </div>
        </div>
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {categories.length>0&&(
              <div className="flex items-center gap-1.5 ml-auto flex-wrap">
                <button onClick={()=>setCatFilter("")} className={`px-2.5 py-1 rounded-lg text-[11px] font-semibold transition-colors ${!catFilter?"bg-indigo-600 text-white":"bg-[#F1F3F9] dark:bg-white/5 text-[#4B5168] dark:text-[#9DA3BB] hover:bg-indigo-50"}`}>All</button>
                {categories.map(cat=>(<button key={cat} onClick={()=>setCatFilter(cat)} className={`px-2.5 py-1 rounded-lg text-[11px] font-semibold transition-colors ${catFilter===cat?"bg-indigo-600 text-white":"bg-[#F1F3F9] dark:bg-white/5 text-[#4B5168] dark:text-[#9DA3BB] hover:bg-indigo-50"}`}>{cat}</button>))}
              </div>
            )}
            <p className="text-[11px] text-[#8B92A9]">{configured.length} with data · Click to expand</p>
          </div>
          {camps.length===0&&<div className="bg-white dark:bg-[#11131C] border border-[#E4E7EF] dark:border-[#1E2133] rounded-2xl p-10 text-center"><BarChart3 className="w-8 h-8 text-[#C4C9DA] mx-auto mb-3" strokeWidth={1.5}/><p className="text-[13px] font-semibold text-[#0F1117] dark:text-[#DDE1F5]">No Meta campaigns configured</p></div>}
          {byCategory.map(([cat,catCamps])=>(
            <div key={cat}>
              {byCategory.length>1&&(<div className="flex items-center gap-2 py-1.5"><div className="h-px flex-1 bg-[#E4E7EF] dark:bg-[#1E2133]"/><span className="text-[10px] font-bold uppercase tracking-wider px-3 py-1 rounded-full bg-indigo-50 dark:bg-indigo-950/30 text-indigo-600 dark:text-indigo-400">{cat} · {catCamps.length} campaign{catCamps.length!==1?"s":""}</span><div className="h-px flex-1 bg-[#E4E7EF] dark:bg-[#1E2133]"/></div>)}
...
function ConvBar({total,converted,inProgress,notInt}){
  ...
  const segs=[{v:converted,color:"#10B981",label:"Converted"},{v:inProgress,color:"#F59E0B",label:"In Progress"},{v:notInt,color:"#EF4444",label:"Not Interested"}];
  return(
    <div>
      ...
      <div className="flex items-center gap-2 mt-1 flex-wrap">
        {segs.map(s=><span key={s.label} className="inline-flex items-center gap-1 text-[9px] text-[#8B92A9]"><span className="w-1.5 h-1.5 rounded-full" style={{background:s.color}}/>{s.label}: <b className="text-[#4B5168] dark:text-[#9DA3BB]">{s.v}</b> ({pct(s.v)}%)</span>)}
      </div>
    </div>
  );
}
  ...
  return(
    <div className="bg-white dark:bg-[#11131C] border border-[#E4E7EF] dark:border-[#1E2133] rounded-2xl overflow-hidden">
      <div className="flex items-center gap-3 px-4 py-4 cursor-pointer hover:bg-[#F8F9FC] dark:hover:bg-white/[0.02] transition-colors" onClick={()=>setOpen(!open)}>
        <div className="w-9 h-9 rounded-xl flex items-center justify-center text-white font-bold text-[13px] shrink-0" style={{background:CAMP_COLORS[ci%CAMP_COLORS.length]}}>{ci+1}</div>
        <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center gap-3 shrink-0">
          <div className="text-right"><p className="text-[16px] font-extrabold text-emerald-600">{camp.converted}</p><p className="text-[9px] text-[#8B92A9] uppercase tracking-wide">Converted</p></div>
          <div className="text-right"><p className="text-[16px] font-extrabold text-[#0F1117] dark:text-[#DDE1F5]">{camp.convRate}%</p><p className="text-[9px] text-[#8B92A9] uppercase tracking-wide">Conv. Rate</p></div>
          {open?<ChevronUp className="w-4 h-4 text-[#8B92A9]"/>:<ChevronDown className="w-4 h-4 text-[#8B92A9]"/>}
        </div>
      </div>
      ...
      {open&&(
        <div className="border-t border-[#E4E7EF] dark:border-[#1E2133]">
          <div className="grid grid-cols-8 bg-[#F8F9FC] dark:bg-[#0D0F14] px-4 py-2 text-[9px] font-bold uppercase tracking-wider text-[#8B92A9] gap-2">
            <div className="col-span-2">Ad Set / Source</div><div className="text-center">Total</div><div className="text-center text-blue-600">New</div><div className="text-center text-amber-600">In Progress</div><div className="text-center text-purple-600">Verif.</div><div className="text-center text-emerald-600">Converted</div><div className="text-center text-rose-500">Not Int.</div>
          </div>
          {camp.adSets.map((adSet,ai)=>(
            <div key={ai} className="border-t border-[#F1F3F9] dark:border-white/5">
              <div className="grid grid-cols-8 px-4 py-3 gap-2 hover:bg-[#F8F9FC] dark:hover:bg-white/[0.015] transition-colors">
                ...
                <div className="text-center"><p className="text-[14px] font-extrabold text-[#0F1117] dark:text-[#DDE1F5]">{adSet.total}</p><p className="text-[9px] font-bold text-[#8B92A9]">{adSet.convRate}% conv</p></div>
              ...
              <div className="px-4 pb-2"><ConvBar total={adSet.total} converted={adSet.converted} inProgress={adSet.inProgress} notInt={adSet.notInt}/></div>
            </div>
          ))}
        </div>
      )}
...
function LeadsIntelligenceTab({from,to,refreshKey}){
  const[data,setData]=useState(null);
  const[loading,setLoad]=useState(false);
  const[error,setError]=useState("");
  ...
  // FIX: refreshKey triggers reload
  ...
  const totals=useMemo(()=>{
    ...
    return{total:leads.length,converted:leads.filter(l=>l.status==="Converted").length,inProgress:leads.filter(l=>l.status==="In Progress").length,notInt:leads.filter(l=>l.status==="Not Interested").length,newLeads:leads.filter(l=>l.status==="New").length};
  },[data]);
  ...
  return(
    <div className="space-y-4">
      <div className="flex items-center gap-3 flex-wrap">
        ...
        <div><p className="text-[15px] font-extrabold text-[#0F1117] dark:text-[#F0F2FA] leading-tight">Leads Intelligence</p><p className="text-[11px] text-[#8B92A9]">Ad-level lead attribution · conversion tracking · lead details</p></div>
        <div className="ml-auto flex items-center gap-2 flex-wrap">
          <select value={campFilter} onChange={e=>{setCampFilter(e.target.value);}} className="text-[11px] bg-white dark:bg-[#11131C] border border-[#E4E7EF] dark:border-[#1E2133] rounded-lg px-2.5 py-1.5 focus:outline-none text-[#4B5168] dark:text-[#9DA3BB]">
      ...
      {data&&(
        <div className="grid grid-cols-2 sm:grid-cols-5 gap-2">
          {[{label:"Total Leads",value:totals.total,tint:"#6366F1"},{label:"New",value:totals.newLeads,tint:"#3B82F6"},{label:"In Progress",value:totals.inProgress,tint:"#F59E0B"},{label:"Converted",value:totals.converted,tint:"#10B981"},{label:"Not Interested",value:totals.notInt,tint:"#EF4444"}].map(c=>(
      ...
      {data&&totals.total>0&&(
        <div className="rounded-2xl p-4 bg-gradient-to-br from-emerald-600 to-teal-600 text-white shadow-md">
          ...
          <p className="text-[13px] leading-relaxed"><b>{totals.total}</b> leads — <b className="text-emerald-200">{totals.converted} converted</b> ({totals.total>0?Math.round((totals.converted/totals.total)*100):0}% rate), <b>{totals.inProgress}</b> in progress, <b className="text-rose-300">{totals.notInt}</b> not interested.</p>
        </div>
      )}
      ...
      <div className="flex gap-1 bg-white dark:bg-[#11131C] border border-[#E4E7EF] dark:border-[#1E2133] rounded-xl p-1">
        {[["adlevel","Ad-Level Breakdown"],["converting","Converting Leads"],["all","All Leads"]].map(([v,l])=>(
          <button key={v} onClick={()=>{setView(v);setSearch("");}} className={`flex-1 py-1.5 rounded-lg text-[11px] font-bold transition-colors ${view===v?"bg-indigo-600 text-white shadow":"text-[#8B92A9] hover:text-[#4B5168] dark:hover:text-[#DDE1F5]"}`}>{l}</button>
        ))}
      </div>

      {view==="adlevel"&&data&&(
        <div className="space-y-3">
          {(!data.adLevel||data.adLevel.length===0)&&<div className="bg-white dark:bg-[#11131C] border border-[#E4E7EF] dark:border-[#1E2133] rounded-2xl p-10 text-center text-[12px] text-[#8B92A9]">No lead data in this period.</div>}
          {(data.adLevel||[]).map((camp,ci)=><CampaignAdCard key={camp.campaign+ci} camp={camp} ci={ci}/>)}
        </div>
      )}
      ...
      {(view==="converting"||view==="all")&&data&&(
        <div className="space-y-2">
          <div className="flex items-center gap-2">
            <div className="relative flex-1 max-w-xs"><Search className="w-3.5 h-3.5 text-[#8B92A9] absolute left-2.5 top-1/2 -translate-y-1/2"/><input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search name, phone, campaign…" className="w-full text-[12px] pl-8 pr-3 py-2 rounded-xl border border-[#E4E7EF] dark:border-[#1E2133] bg-white dark:bg-[#11131C] focus:outline-none text-[#0F1117] dark:text-[#DDE1F5]"/></div>
            <span className="text-[11px] text-[#8B92A9] ml-auto">{filteredLeads.length} leads</span>
          </div>
          <div className="bg-white dark:bg-[#11131C] border border-[#E4E7EF] dark:border-[#1E2133] rounded-2xl overflow-x-auto">
            <table className="w-full border-collapse">
              <thead><tr className="border-b border-[#E4E7EF] dark:border-[#1E2133] bg-[#F8F9FC] dark:bg-[#0D0F14]">
                {["Lead","Contact","Campaign","Ad Set","Source","Status","Agent","Date","Last Remark"].map(h=>(<th key={h} className="text-[10px] font-bold uppercase tracking-wider text-[#8B92A9] px-3 py-2.5 text-left whitespace-nowrap">{h}</th>))}
              </tr></thead>
              <tbody>
                {filteredLeads.slice(0,200).map((l,i)=>(
                  <tr key={l._id||i} className="border-b border-[#F1F3F9] dark:border-white/5 last:border-0 hover:bg-[#F8F9FC] dark:hover:bg-white/[0.02] transition-colors">
                    <td className="px-3 py-3"><div className="flex items-center gap-2"><div className="w-7 h-7 rounded-full flex items-center justify-center text-white text-[11px] font-bold shrink-0" style={{background:l.status==="Converted"?"#10B981":l.status==="In Progress"?"#F59E0B":"#6366F1"}}>{(l.name||"?").charAt(0).toUpperCase()}</div><div><p className="text-[12px] font-semibold text-[#0F1117] dark:text-[#DDE1F5]">{l.name}</p>{l.language&&<span className="text-[9px] font-bold px-1.5 py-0.5 rounded-full bg-indigo-50 text-indigo-600 dark:bg-indigo-950/30">{l.language}</span>}</div></div></td>
                    <td className="px-3 py-3"><p className="text-[12px] tabular-nums text-[#4B5168] dark:text-[#9DA3BB]">{l.mobile}</p>{l.email&&<p className="text-[10px] text-[#8B92A9] truncate max-w-[120px]">{l.email}</p>}</td>
                    <td className="px-3 py-3 text-[11px] text-[#4B5168] dark:text-[#9DA3BB] max-w-[130px]"><p className="truncate font-medium" title={l.campaign}>{l.campaign||"—"}</p></td>
                    <td className="px-3 py-3 text-[11px] text-[#8B92A9] max-w-[120px]"><p className="truncate" title={l.adSet}>{l.adSet||"—"}</p></td>
                    <td className="px-3 py-3"><span className="text-[10px] font-semibold px-2 py-0.5 rounded-full bg-[#F8F9FC] dark:bg-white/5 text-[#4B5168] dark:text-[#9DA3BB]">{l.source||"—"}</span></td>
                    ...
                    <td className="px-3 py-3 text-[11px] text-[#8B92A9] max-w-[180px]"><p className="line-clamp-2" title={l.remark}>{l.remark||"—"}</p></td>
                  </tr>
                ))}
...
function MetaAdsTab({from,to,refreshKey}){
  ...
  const byCampaign=useMemo(()=>{const map={};(data?.ads||[]).forEach(a=>{const k=a.campaignName||"Unknown";if(!map[k])map[k]=[];map[k].push(a);});return Object.entries(map).sort((a,b)=>b[1].reduce((s,x)=>s+(x.metrics.spend||0),0)-a[1].reduce((s,x)=>s+(x.metrics.spend||0),0));},[data]);
    ...
    <div className="space-y-4">
      {t.spend>0&&(
        <div className="rounded-2xl p-4 bg-gradient-to-br from-blue-600 via-indigo-600 to-violet-600 text-white shadow-md">
          ...
          <p className="text-[13px] leading-relaxed">Spent <b>₹{Number(t.spend).toLocaleString("en-IN",{maximumFractionDigits:0})}</b> reaching <b>{Number(t.reach).toLocaleString("en-IN")}</b> people with <b>{Number(t.impressions).toLocaleString("en-IN")}</b> impressions and <b>{Number(t.clicks).toLocaleString("en-IN")}</b> clicks (<b>{ctr}% CTR</b>). <span className="text-emerald-300 font-bold">{goodAds} ads</span> performing well · <span className="text-rose-300 font-bold">{needsAds} ads</span> need attention.</p>
        </div>
      )}

      {t.spend>0&&(
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5">
          {[{icon:IndianRupee,label:"Spend",value:"₹"+Number(t.spend).toLocaleString("en-IN",{maximumFractionDigits:0}),tint:"#EF4444"},{icon:Eye,label:"Impressions",value:Number(t.impressions).toLocaleString("en-IN"),tint:"#6366F1"},{icon:Users,label:"Reach",value:Number(t.reach).toLocaleString("en-IN"),tint:"#10B981"},{icon:MousePointerClick,label:"Clicks",value:Number(t.clicks).toLocaleString("en-IN"),tint:"#0EA5E9"},{icon:Percent,label:"CTR",value:`${ctr}%`,tint:"#F59E0B"},{icon:BarChart3,label:"Campaigns",value:byCampaign.length,tint:"#8B5CF6"},{icon:CheckCircle2,label:"Good Ads",value:goodAds,tint:"#10B981"},{icon:AlertCircle,label:"Need Attention",value:needsAds,tint:"#EF4444"}].map(c=>(
            <div key={c.label} className="relative overflow-hidden bg-white dark:bg-[#11131C] border border-[#E4E7EF] dark:border-[#1E2133] rounded-2xl p-3.5 hover:shadow-md transition-all" style={{background:`linear-gradient(135deg,${c.tint}14 0%,transparent 70%)`}}>
              <div className="flex items-center gap-2 mb-1.5"><span className="w-6 h-6 rounded-lg flex items-center justify-center" style={{background:`${c.tint}22`}}><c.icon className="w-3.5 h-3.5" style={{color:c.tint}}/></span><span className="text-[9px] font-bold uppercase tracking-wider text-[#8B92A9]">{c.label}</span></div>
              <p className="text-[18px] font-extrabold text-[#0F1117] dark:text-[#F0F2FA] leading-none">{c.value}</p>
            </div>
          ))}
        </div>
      ...
      <div className="flex items-center gap-2 flex-wrap">
        ...
        <div className="flex gap-1 bg-[#F1F3F9] dark:bg-white/5 rounded-xl p-1">
          {[["campaigns","By Campaign"],["table","All Ads"]].map(([v,l])=>(<button key={v} onClick={()=>setView(v)} className={`px-3 py-1.5 rounded-lg text-[11px] font-bold transition-colors ${view===v?"bg-white dark:bg-[#11131C] text-indigo-600 shadow-sm":"text-[#8B92A9] hover:text-[#4B5168]"}`}>{l}</button>))}
        </div>
        {view==="table"&&(<div className="relative ml-2"><Search className="w-3.5 h-3.5 text-[#8B92A9] absolute left-2.5 top-1/2 -translate-y-1/2"/><input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search…" className="text-[11px] pl-8 pr-3 py-1.5 rounded-xl border border-[#E4E7EF] dark:border-[#1E2133] bg-white dark:bg-[#11131C] focus:outline-none text-[#0F1117] dark:text-[#DDE1F5] w-44"/></div>)}
        {loading&&<Loader2 className="w-3.5 h-3.5 animate-spin text-[#8B92A9] ml-auto"/>}
      </div>

      {view==="campaigns"&&(<div className="space-y-3">{byCampaign.length===0&&<div className="bg-white dark:bg-[#11131C] border border-[#E4E7EF] dark:border-[#1E2133] rounded-2xl p-10 text-center text-[12px] text-[#8B92A9]">No ads in this period.</div>}{byCampaign.map(([name,ads],ci)=>{const spend=ads.reduce((s,a)=>s+(a.metrics.spend||0),0);const impr=ads.reduce((s,a)=>s+(a.metrics.impressions||0),0);const clicks=ads.reduce((s,a)=>s+(a.metrics.clicks||0),0);const reach=ads.reduce((s,a)=>s+(a.metrics.reach||0),0);const ctrC=impr>0?(clicks/impr)*100:0;const active=ads.filter(a=>a.status==="ACTIVE").length;const goodC=ads.filter(a=>scoreAd(a.metrics).label==="Good").length;const needsC=ads.filter(a=>scoreAd(a.metrics).label==="Needs Attention").length;const campScore=scoreAd({ctr:ctrC,cpc:clicks>0?spend/clicks:0,frequency:0});const CSI=campScore.icon;return(<div key={name+ci} className="bg-white dark:bg-[#11131C] border border-[#E4E7EF] dark:border-[#1E2133] rounded-2xl overflow-hidden"><div className="flex items-center gap-3 px-4 py-3.5 cursor-pointer hover:bg-[#F8F9FC] dark:hover:bg-white/[0.02]" onClick={()=>{}}><div className="w-9 h-9 rounded-xl flex items-center justify-center text-white font-bold text-[13px] shrink-0" style={{background:COLORS[ci%COLORS.length]}}>{ci+1}</div><div className="flex-1 min-w-0"><p className="text-[13px] font-bold text-[#0F1117] dark:text-[#DDE1F5] truncate">{name}</p><p className="text-[10px] text-[#8B92A9]">{ads.length} ads · {active} active · <span className="text-emerald-600">{goodC} good</span> · <span className="text-rose-500">{needsC} need attention</span></p></div><div className="flex items-center gap-2 shrink-0"><div className="text-right hidden sm:block"><p className="text-[12px] font-bold text-[#0F1117] dark:text-[#DDE1F5]">₹{Number(spend).toLocaleString("en-IN",{maximumFractionDigits:0})}</p><p className="text-[9px] text-[#8B92A9]">Spent</p></div><span className={`inline-flex items-center gap-1 text-[10px] font-bold px-2 py-0.5 rounded-full ${campScore.bg}`} style={{color:campScore.color}}><CSI className="w-3 h-3"/>{campScore.label}</span></div></div><div className="grid grid-cols-4 sm:grid-cols-8 bg-[#F8F9FC] dark:bg-[#0D0F14] border-t border-[#E4E7EF] dark:divide-[#1E2133] divide-x divide-[#E4E7EF] dark:divide-[#1E2133]">{[["Spend",`₹${Number(spend).toLocaleString("en-IN",{maximumFractionDigits:0})}`],["Impressions",Number(impr).toLocaleString("en-IN")],["Reach",Number(reach).toLocaleString("en-IN")],["Clicks",Number(clicks).toLocaleString("en-IN")],["CTR",`${ctrC.toFixed(2)}%`],["CPM",`₹${impr>0?(spend/impr*1000).toFixed(2):0}`],["CPC",`₹${clicks>0?(spend/clicks).toFixed(2):0}`],["Ads",ads.length]].map(([k,v])=>(<div key={k} className="flex flex-col px-2 py-2"><span className="text-[9px] font-bold uppercase tracking-wider text-[#8B92A9] mb-0.5">{k}</span><span className="text-[11px] font-bold text-[#0F1117] dark:text-[#DDE1F5] tabular-nums">{v}</span></div>))}</div><div>{ads.map((ad,ai)=><MktAdRow key={ad.adId||ai} ad={ad}/>)}</div></div>);})}</div>)}
      ...
      {view==="table"&&(
        <div className="bg-white dark:bg-[#11131C] border border-[#E4E7EF] dark:border-[#1E2133] rounded-2xl overflow-x-auto">
          <table className="w-full border-collapse">
            ...
            <tbody>
              {filtered.map((ad,i)=>{const m=ad.metrics||{},sc=scoreAd(m),SI=sc.icon;const stCls={"ACTIVE":"bg-emerald-50 text-emerald-700","PAUSED":"bg-amber-50 text-amber-700","CAMPAIGN_PAUSED":"bg-slate-100 text-slate-500","DELETED":"bg-rose-50 text-rose-600"}[ad.status]||"bg-slate-100 text-slate-500";return(<tr key={ad.adId||i} className="border-b border-[#F1F3F9] dark:border-white/5 last:border-0 hover:bg-[#F8F9FC] dark:hover:bg-white/[0.02]"><td className="px-3 py-2.5 text-[12px] font-semibold text-[#0F1117] dark:text-[#DDE1F5] max-w-[150px] truncate" title={ad.adName}>{ad.adName}</td><td className="px-3 py-2.5 text-[11px] text-[#8B92A9] max-w-[120px] truncate">{ad.campaignName}</td><td className="px-3 py-2.5"><span className={`text-[10px] font-bold px-2 py-0.5 rounded-full capitalize ${stCls}`}>{(ad.status||"").toLowerCase().replace(/_/g," ")}</span></td><td className="px-3 py-2.5"><span className={`inline-flex items-center gap-1 text-[10px] font-bold px-2 py-0.5 rounded-full ${sc.bg}`} style={{color:sc.color}}><SI className="w-3 h-3"/>{sc.label}</span></td><td className="px-3 py-2.5 text-right text-[12px] tabular-nums font-semibold text-[#0F1117] dark:text-[#DDE1F5]">₹{Number(m.spend||0).toLocaleString("en-IN",{maximumFractionDigits:0})}</td><td className="px-3 py-2.5 text-right text-[11px] tabular-nums text-[#8B92A9]">{Number(m.impressions||0).toLocaleString("en-IN")}</td><td className="px-3 py-2.5 text-right text-[11px] tabular-nums text-[#8B92A9]">{Number(m.clicks||0).toLocaleString("en-IN")}</td><td className="px-3 py-2.5 text-right text-[12px] tabular-nums font-bold" style={{color:Number(m.ctr)>=2?"#10B981":Number(m.ctr)>=1?"#F59E0B":"#EF4444"}}>{Number(m.ctr||0).toFixed(2)}%</td><td className="px-3 py-2.5 text-right text-[11px] tabular-nums text-[#8B92A9]">₹{Number(m.cpc||0).toFixed(2)}</td><td className="px-3 py-2.5 text-right text-[12px] tabular-nums font-bold" style={{color:Number(m.frequency)>5?"#EF4444":Number(m.frequency)>3?"#F59E0B":"#10B981"}}>{Number(m.frequency||0).toFixed(1)}×</td></tr>);})}
...
export default function MarketingDashboard() {
  ...
  // FIX: single shared refreshKey — incrementing it propagates refresh to all tabs
  ...
  const handleRefresh = useCallback(()=>{
    // FIX: increment key to trigger all tab reloads simultaneously
    setRefreshKey(k=>k+1);
  ...
  // FIX: auto-refresh now calls handleRefresh so ALL tabs update
  useEffect(()=>{
    if(!autoMin) return;
  ...
  return(
    <div className="min-h-screen bg-[#F8F9FC] dark:bg-[#0D0F14]">
      ...
      <nav className="h-14 bg-white dark:bg-[#11131C] border-b border-[#E4E7EF] dark:border-[#1E2133] px-4 md:px-6 flex items-center gap-3 sticky top-0 z-50">
        ...
        {/* Tab nav in header */}
        <div className="hidden md:flex gap-0.5 bg-[#F1F3F9] dark:bg-white/5 rounded-xl p-1 mx-4">
          {tabDef.map(([v,l])=>(
            <button key={v} onClick={()=>setActiveTab(v)}
              className={`px-3 py-1.5 rounded-lg text-[11px] font-bold transition-colors ${activeTab===v?"bg-white dark:bg-[#11131C] text-indigo-600 shadow-sm":"text-[#8B92A9] hover:text-[#4B5168] dark:hover:text-[#DDE1F5]"}`}>{l}</button>
          ))}
        </div>

        <div className="ml-auto flex items-center gap-2">
          {lastRefreshed&&<span className="text-[10px] text-[#8B92A9] hidden lg:inline">Updated {lastRefreshed.toLocaleTimeString("en-IN",{hour:"2-digit",minute:"2-digit"})}</span>}
          ...
          {/* FIX: single Refresh button now refreshes ALL tabs */}
          <button onClick={handleRefresh} disabled={loading}
            className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-indigo-600 hover:bg-indigo-700 disabled:opacity-50 text-white text-[11px] font-bold">
          ...
          <button onClick={logout} className="w-8 h-8 rounded-lg border border-[#E4E7EF] dark:border-[#1E2133] flex items-center justify-center text-[#8B92A9] hover:text-rose-500 transition-colors" title="Logout"><LogOut className="w-3.5 h-3.5"/></button>
        </div>
      </nav>

      <div className="max-w-[1400px] mx-auto px-4 md:px-6 py-5 space-y-4">
        {/* Mobile tab nav */}
        <div className="flex gap-1 bg-white dark:bg-[#11131C] border border-[#E4E7EF] dark:border-[#1E2133] rounded-2xl p-1.5 md:hidden">
          {tabDef.map(([v,l])=>(<button key={v} onClick={()=>setActiveTab(v)} className={`flex-1 py-2 rounded-xl text-[11px] font-bold transition-colors ${activeTab===v?"bg-indigo-600 text-white shadow":"text-[#8B92A9] hover:text-[#4B5168]"}`}>{l}</button>))}
        </div>
        ...
        {/* ── TAB CONTENT ────────────────────────────────────────────────── */}
        {/* FIX: refreshKey passed to each tab so they all reload on Refresh */}
        {activeTab==="meta"&&<MetaCampaignsTab from={from} to={to} refreshKey={refreshKey}/>}
        {activeTab==="leads"&&<LeadsIntelligenceTab from={from} to={to} refreshKey={refreshKey}/>}
        {activeTab==="adlevel"&&<MetaAdsTab from={from} to={to} refreshKey={refreshKey}/>}
        ...
        {activeTab==="overview"&&<div className="space-y-4">
          ...
          {/* Source + campaign filters (overview-specific) */}
          <div className="bg-white dark:bg-[#11131C] border border-[#E4E7EF] dark:border-[#1E2133] rounded-2xl px-4 py-3 flex items-center gap-3 flex-wrap">
            <span className="text-[10px] font-bold uppercase tracking-wider text-[#8B92A9]">Filter</span>
            <select value={source} onChange={e=>setSource(e.target.value)} className="text-[11px] bg-white dark:bg-[#11131C] border border-[#E4E7EF] dark:border-[#1E2133] rounded-lg px-2 py-1.5 focus:outline-none text-[#4B5168] dark:text-[#9DA3BB]">
              <option value="">All sources</option>
              {(data?.filters?.sources||[]).map(s=><option key={s} value={s}>{s}</option>)}
            </select>
            <select value={campaign} onChange={e=>setCampaign(e.target.value)} className="text-[11px] bg-white dark:bg-[#11131C] border border-[#E4E7EF] dark:border-[#1E2133] rounded-lg px-2 py-1.5 focus:outline-none text-[#4B5168] dark:text-[#9DA3BB]">
              <option value="">All campaigns</option>
              {(data?.filters?.campaigns||[]).map(c=><option key={c} value={c}>{c}</option>)}
            </select>
          ...
          {data&&<>
            ...
            <div className="rounded-2xl p-4 bg-gradient-to-br from-indigo-600 via-violet-600 to-purple-700 text-white shadow-lg">
              <p className="text-[10px] font-bold uppercase tracking-wider opacity-70 mb-1">{data.range?.from} → {data.range?.to}</p>
              <p className="text-[13px] leading-relaxed">
                <b>{num(k.totalLeads)}</b> leads — <b>{num(k.converted)}</b> converted (<b>{pct(k.conversionRate)}</b>), <b>{num(k.inProgress)}</b> in progress, <b>{num(k.newLeads)}</b> new. <b>{num(data.followups?.today)}</b> follow-ups due today, <b>{num(data.followups?.missed)}</b> missed.
              </p>
            </div>
            ...
            <div className="grid grid-cols-3 gap-2.5">
              {[{label:"Today's Follow-ups",count:data.followups?.today,color:"#F97316",icon:Clock},{label:"Upcoming Follow-ups",count:data.followups?.upcoming,color:"#0EA5E9",icon:Calendar},{label:"Missed Follow-ups",count:data.followups?.missed,color:"#EF4444",icon:AlertTriangle}].map(f=>(
                <div key={f.label} className="bg-white dark:bg-[#11131C] border border-[#E4E7EF] dark:border-[#1E2133] rounded-2xl p-4">
                  <div className="w-8 h-8 rounded-xl flex items-center justify-center mb-2" style={{background:`${f.color}22`}}><f.icon className="w-4 h-4" style={{color:f.color}}/></div>
