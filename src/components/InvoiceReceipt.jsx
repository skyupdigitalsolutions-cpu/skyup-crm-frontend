// ─────────────────────────────────────────────────────────────────────────────
//  InvoiceReceipt.jsx — Professional GST tax invoice (card layout)
//  Columns: SL.No. · Description · Tax Rate · Qty · Rate · Amount
//  • Every line `amount` is GST-INCLUSIVE; taxable + tax derived from it.
//  • Location-aware GST: customer state == seller (29) → CGST+SGST, else IGST.
//  • Tax breakdown always sums EXACTLY to the inclusive total.
// ─────────────────────────────────────────────────────────────────────────────
...
export default function InvoiceReceipt({ invoice, company: companyProp, onClose }) {
  ...
  const metaLabel = { fontSize: 11, color: MUTED, fontWeight: 700, padding: "2px 0", paddingRight: 12, whiteSpace: "nowrap" };
  const metaValue = { fontSize: 11, fontWeight: 600, padding: "2px 0", whiteSpace: "nowrap" };
      ...
      <div className="w-full max-w-[640px]">
        ...
        <div ref={printRef}>
          <div style={{
            ...
            {/* ── Header row: seller (left) + invoice meta (right) ── */}
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 20 }}>
              ...
              <div style={{ display: "flex", gap: 12 }}>
                {COMPANY_LOGO_URL ? (
                  ...
                  <div style={{
                    width: 34, height: 34, borderRadius: 9, background: "#1d4ed8", color: "#fff",
              ...
              {/* Right: INVOICE title + status + meta table */}
                  ...
                  <table style={{ borderCollapse: "collapse" }}>
                    <tbody>
                      <tr>
                        ...
                        <td style={{ ...metaValue, fontWeight: 800 }}>{invoice.invoiceId}</td>
                      </tr>
                      <tr>
                        <td style={metaLabel}>DATE</td>
                        <td style={metaValue}>{invoice.date}</td>
                      </tr>
                      {invoice.transactionId && (
                        <tr>
                          <td style={metaLabel}>TXN ID</td>
                          <td style={metaValue}>{invoice.transactionId}</td>
                        </tr>
                      )}
                      <tr>
                        <td style={metaLabel}>PAID VIA</td>
                        <td style={metaValue}>{invoice.paymentMethod || "Razorpay"}</td>
                      </tr>
                    </tbody>
                  </table>
                </div>
            ...
            <div>
              ...
              <div style={{ fontSize: 14, fontWeight: 700, color: "#000" }}>{customer.name || "—"}</div>
              {customer.email && <div style={{ fontSize: 12, color: "#1d4ed8", marginTop: 2 }}>{customer.email}</div>}
              {customer.address && <div style={{ fontSize: 12, marginTop: 3 }}>{customer.address}</div>}
              <div style={{ fontSize: 12, marginTop: 4 }}>
                {customer.gstin && <><b>GSTIN:</b> {customer.gstin}</>}
                {custCode && <> &nbsp;·&nbsp; <b>State:</b> {custCode}</>}
              </div>
            </div>
            ...
            <div style={{ marginTop: 20 }}>
              <div style={{
                ...
                borderBottom: "2px solid #000", fontSize: 11, fontWeight: 700,
              ...
              {rows.map((r) => (
                <div key={r.sl} style={{
                  ...
                  <div>
                    ...
                    {r.sub && <div style={{ fontSize: 11, color: MUTED, marginTop: 3 }}>{r.sub}</div>}
                  </div>
            ...
            <div style={{ display: "flex", justifyContent: "flex-end", marginTop: 18 }}>
              <div style={{ width: 280 }}>
                ...
                <div style={{ display: "flex", justifyContent: "space-between", padding: "3px 0" }}>
                  ...
                  <span style={{ fontSize: 16, fontWeight: 800, color: "#1d4ed8", fontVariantNumeric: "tabular-nums" }}>Rs. {fmt(total)}</span>
                </div>
            ...
            {activated.length > 0 && (
              <div style={{ marginTop: 20, border: "1px solid #e6e8ee", borderRadius: 12, padding: "14px 16px" }}>
                ...
                <div style={{ marginTop: 9 }}>
                  {activated.map((c, i) => (
                    <span key={i} style={{ fontSize: 12, fontWeight: 600, color: "#1d4ed8", marginRight: 18 }}>{c}</span>
                  ))}
                </div>
              </div>
            )}
            ...
            <div style={{ textAlign: "center", fontSize: 9.5, color: MUTED, marginTop: 22, lineHeight: 1.7 }}>
              ...
              For queries: <span style={{ color: "#1d4ed8" }}>{company.email}</span>
            </div>
...
function TypeTag({ type }) {
  ...
  const col = isPlan ? "#1d4ed8" : "#047857";
  return (
    <span style={{
