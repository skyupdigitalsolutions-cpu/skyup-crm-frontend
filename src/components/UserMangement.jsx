...
import { getStoredUser } from "../data/dataService";// UserManagement.jsx — FIXED
...
export default function UserManagement({
  ...
  // ── FIX: Show Add Admin button for superadmin role OR isCompanySuperAdmin flag ──
  const role = getRole();
      ...
      <div className={`mb-6 rounded-2xl border-2 p-4 flex flex-wrap items-center justify-between gap-4 ${cfg.borderColor} ${cfg.bgColor}`}>
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className={`text-[11px] mt-0.5 ${cfg.statColor}`}>{cfg.price} · Up to {cfg.maxAdmins} admins · {cfg.maxUsers} users</p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <div className="bg-white dark:bg-[#1A1D27] border border-[#E4E7EF] dark:border-[#262A38] rounded-2xl overflow-hidden">
          <div className="px-5 py-4 border-b border-[#E4E7EF] dark:border-[#262A38] flex items-center justify-between">
            ...
            {/* ── FIXED: button now visible for superadmin role too ── */}
            {isCompanySuperAdmin && (
              <button
