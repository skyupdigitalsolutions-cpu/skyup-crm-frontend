...
function AddLeadModal({ onClose, onAdd }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setErrors({ submit: msg, ...(isDup ? { mobile: msg } : {}) });
    } finally {
      setSubmitting(false);
    }
  };
...
function ImportCSVModal({ onClose, onImported }) {
  ...
  const [result,    setResult]    = useState(null); // { savedCount, errorCount, errors[] }
  ...
  const handleFile = async (e) => {
    ...
    try {
      ...
      // ── Client-side duplicate detection within the CSV itself ──────────────
      ...
      for (let i = 1; i < lines.length; i++) {
        ...
        // Check duplicate within the CSV
        const dupKey = normalized.replace(/^(91|1)/, ""); // strip leading country code for comparison
        ...
        {!result ? (
          <>
            ...
            <div className="bg-[#EFF6FF] dark:bg-[#1A2540] border border-blue-100 dark:border-blue-900/40 rounded-xl p-4 mb-5">
              ...
              <p className="text-[12px] text-[#4B5168] dark:text-[#9DA3BB] mb-1">Required columns: <code className="font-mono bg-white dark:bg-[#0D0F14] px-1 rounded">name</code>, <code className="font-mono bg-white dark:bg-[#0D0F14] px-1 rounded">mobile</code></p>
              <p className="text-[12px] text-[#4B5168] dark:text-[#9DA3BB]">Optional: email, source, campaign, status, remark</p>
              <p className="text-[11px] text-[#8B92A9] mt-2">• Duplicate numbers (with or without +91) are automatically skipped<br/>• Leads round-robin assigned to your team</p>
            </div>
            ...
            <div className="flex gap-2">
              ...
              <button onClick={() => importRef.current?.click()} disabled={importing}
                ...
                {importing
                  ? <><svg className="w-3.5 h-3.5 animate-spin" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"/><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v8z"/></svg>Importing…</>
                  : <><svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12"/></svg>Choose CSV File</>
                }
              </button>
            </div>
          </>
...
export default function AdminLeadsPage() {
  ...
  // Called after adding a single lead — prepend to list immediately
      ...
      <div className="flex items-center justify-between mb-6 flex-wrap gap-3">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <button onClick={exportToCSV} disabled={!displayed.length}
            ...
            Export CSV
            {displayed.length > 0 && (
              <span className="bg-emerald-100 dark:bg-emerald-950/60 text-emerald-700 dark:text-emerald-300 text-[10px] font-bold px-1.5 py-0.5 rounded-full">{displayed.length}</span>
            )}
          </button>
          {/* Refresh */}
      ...
      <div className="bg-white dark:bg-[#1A1D27] border border-[#E4E7EF] dark:border-[#262A38] rounded-2xl p-4 mb-4">
        <div className="flex flex-wrap gap-3 items-center">
          ...
          {(search || filterSt !== "All" || filterAgent !== "All" || filterSrc !== "All" || filterTemp !== "All" || dateFrom || dateTo) && (
            <button onClick={clearFilters} className="px-3 py-2 rounded-xl border border-red-200 dark:border-red-800 text-red-500 text-[12px] font-semibold hover:bg-red-50 dark:hover:bg-red-950/30 transition">✕ Clear</button>
          )}
        </div>
      ...
      <div className="bg-white dark:bg-[#1A1D27] border border-[#E4E7EF] dark:border-[#262A38] rounded-2xl overflow-hidden">
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-16 gap-3">
            ...
            {allLeads.length === 0 ? (
              <button onClick={() => setShowAdd(true)} className="mt-1 flex items-center gap-1.5 px-4 py-2 rounded-xl bg-[#059669] text-white text-[13px] font-semibold hover:bg-emerald-700 transition">
                <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M12 4v16m8-8H4"/></svg>
                Add first lead
              </button>
            ) : (
