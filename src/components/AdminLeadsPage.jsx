...
// Fetches all transcribed call summaries for a lead and synthesises them
// into one master summary via the backend GPT pipeline.
    ...
    <div className="rounded-xl border border-violet-200 dark:border-violet-800/50 overflow-hidden">
      ...
      >
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <span className="text-[12px] font-bold text-violet-700 dark:text-violet-300">
            Lead AI Summary
          </span>
          <span className="text-[10px] text-violet-500 dark:text-violet-400">
            {data
              ? `${data.summarizedCalls} call${data.summarizedCalls !== 1 ? "s" : ""} combined`
              : "All transcribed calls combined"}
          </span>
          {sent && (
            <span className={`text-[10px] font-semibold px-2 py-0.5 rounded-full ${sentStyle.bg} ${sentStyle.text}`}>
              {sent}
            </span>
          )}
          {temp && tempStyle && (
        ...
        <div className="px-4 py-4 bg-white dark:bg-[#1A1D27] space-y-3">
          ...
          {error && !loading && (
            <div className="space-y-2">
              ...
              <button
                ...
                className="text-[12px] text-violet-600 underline pl-1"
              >
                Retry
              </button>
            </div>
          ...
          {cs && !loading && !error && (
            <>
              ...
              {cs.overallSummary && (
                <div>
                  <p className="text-[10px] font-bold text-[#8B92A9] uppercase tracking-widest mb-1.5">
                    Overall Summary
                  </p>
                  <p className="text-[13px] text-[#4B5168] dark:text-[#9DA3BB] leading-relaxed">
                    {cs.overallSummary}
                  </p>
                </div>
              )}
              ...
              {cs.relationshipStatus && (
                <div className="flex items-start gap-2.5 px-3 py-2.5 rounded-lg bg-[#EEF3FF] dark:bg-[#1A2540]">
                  ...
                  <div>
                    <p className="text-[10px] font-bold text-[#2563EB] dark:text-[#4F8EF7] uppercase tracking-wide mb-0.5">
                      Relationship Status
                    </p>
              ...
              {cs.recommendedNextAction && (
                <div className="flex items-start gap-2.5 px-3 py-2.5 rounded-lg bg-[#F0FDF4] dark:bg-emerald-900/20 border border-emerald-100 dark:border-emerald-900/30">
                  <Check className="w-3.5 h-3.5 text-emerald-500 shrink-0 mt-0.5" />
                  <div>
                    <p className="text-[10px] font-bold text-emerald-600 dark:text-emerald-400 uppercase tracking-wide mb-0.5">
                      Recommended Next Action
                    </p>
