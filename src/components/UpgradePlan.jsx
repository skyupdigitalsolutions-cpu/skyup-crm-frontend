// ─────────────────────────────────────────────────────────────────────────────
//  UpgradePlan.jsx  — full downgrade flow wired + upgrade alert navigation
// ─────────────────────────────────────────────────────────────────────────────
...
const PLAN_LIMITS = {
  starter:    { admins: 1,  users: 10 },
  growth:     { admins: 3,  users: 30 },
  enterprise: { admins: 5,  users: 50 },
};
function planRank(id) { return PLAN_ORDER.indexOf(id ?? "starter"); }
function isDowngradeTo(targetId, currentId) {
  return planRank(targetId) < planRank(currentId);
}
...
async function sendInvoiceEmail({ invoiceId, planName, amount, billingCycle, transactionId }) {
  ...
  try {
    await api.post("/razorpay/notify-invoice", { invoiceId, planName, amount, billingCycle, transactionId, date });
  } catch (err) {
...
function PlanCard({ plan, billing, selected, onUpgrade }) {
  ...
  const btnLabel = (() => {
    ...
    if (isSel) return `Proceed to Pay ₹${price.toLocaleString()}`;
  ...
  return (
    <div
      ...
      style={{ ...cardStyle, borderColor: plan.popular ? "#2563EB" : plan.color }}
    >
      ...
      {plan.isDowngrade && !plan.current && (
        <div className="absolute top-4 right-4">
          <span className="px-2.5 py-1 rounded-full bg-amber-100 dark:bg-amber-900/40 text-amber-700 dark:text-amber-300 text-[10px] font-bold">
            Downgrade
          </span>
        </div>
      )}
...
function useRazorpay() {
  ...
  const openCheckout = useCallback(async ({ orderData, plan, billing, onSuccess, onFailure }) => {
    ...
    if (!loaded) { onFailure("Failed to load Razorpay SDK. Check your internet connection."); return; }
...
export default function UpgradePlan({
  ...
  const [selected,      setSelected]      = useState(null);   // plan id selected for payment
  ...
  const [downgradePlan,   setDowngradePlan]   = useState(null);   // plan object to downgrade to
  ...
  const PLANS_DEF = [
    {
      ...
      locked: [
        ...
        "API / Webhook access", "Call recordings",
  ...
  // Enrich plans with current + isDowngrade flags
  const plans = PLANS_DEF.map((p) => ({
    ...p,
  ...
  // ── handleUpgrade: detects downgrade vs upgrade ───────────────────────────
  function handleUpgrade(plan) {
    if (plan.current) return;

    if (plan.isDowngrade) {
      // Always open the downgrade warning modal — no "click again" needed
      setDowngradePlan(plan);
      setShowDowngrade(true);
      return;
    }

    // Regular upgrade: first click selects, second initiates payment
    if (selected !== plan.id) {
      setSelected(plan.id);
      return;
    }
  ...
  // ── Called by DowngradeWarningModal on confirm ────────────────────────────
  async function handleDowngradeConfirmed(adminsToRemove, usersToRemove) {
    if (!downgradePlan) return;

    // Remove the selected members via API before charging
    try {
      if (adminsToRemove.length > 0) {
        await Promise.all(
          adminsToRemove.map((a) =>
            api.delete(`/admin/${a._id || a.id}`).catch(() => {})
          )
        );
      }
      if (usersToRemove.length > 0) {
        await Promise.all(
          usersToRemove.map((u) =>
            api.delete(`/admin/company/users/${u._id || u.id}`).catch(() => {})
          )
        );
      }
    } catch (err) {
      console.error("[Downgrade] Member removal failed", err);
    }

    // Notify parent (UserManagement) to refresh its lists
    if (onDowngrade) onDowngrade(adminsToRemove, usersToRemove);

    setShowDowngrade(false);
    setDowngradePlan(null);

    // Proceed to Razorpay checkout for the lower-tier plan
    await initiatePayment(downgradePlan, true, adminsToRemove, usersToRemove);
  }

  // ── Payment flow ──────────────────────────────────────────────────────────
  async function initiatePayment(plan, _isDowngrade = false, adminsToRemove = [], usersToRemove = []) {
    setPaying(true);
    setError(null);
    try {
      const { data: orderData } = await api.post("/razorpay/create-order", {
        planId: plan.id,
        billing,
        // Tell backend which members were removed (for audit log / webhooks)
        removedAdmins: adminsToRemove.map((a) => a._id || a.id),
        removedUsers:  usersToRemove.map((u)  => u._id || u.id),
      });
      openCheckout({
        orderData,
        plan,
        ...
        onSuccess: (razorpayResponse) =>
          handlePaymentSuccess(plan, razorpayResponse),
        onFailure: (msg) => {
          setPaying(false);
          if (msg) setError(msg);
          setSelected(null);
  ...
  // Limits for the downgrade modal
  const downgradeLimits = downgradePlan ? PLAN_LIMITS[downgradePlan.id] : { admins: 1, users: 10 };

  return (
    <div className="bg-[#F8F9FC] dark:bg-[#0B0D14] min-h-screen font-poppins px-6 py-8">

      {/* ── Checkout preparing overlay ── */}
      {paying && (
          ...
          <p className="text-[13px] text-[#8B92A9] mt-0.5">
            {currentPlan ? (
              <>
                ...
                <span className="font-semibold text-[#0891B2]">{currentPlan.name} plan</span>
                ...
                <table className="w-full text-[13px]">
                  <thead>
                    <tr className="border-b border-[#E4E7EF] dark:border-[#1E2133] bg-[#F8F9FC] dark:bg-[#0D0F17]">
                      ...
                      {plans.map((p) => (
                        <th key={p.id} className="text-center px-4 py-3 text-[11px] font-bold uppercase tracking-wide whitespace-nowrap" style={{ color: p.color }}>
                          {p.name}
                        </th>
                      ))}
                    </tr>
